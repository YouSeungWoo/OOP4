...
class Geo(pygame.sprite.Sprite):
    ...
    def move(self, layer, gamespeed):
        ...
        if self.rect.bottom >= height+27: #밑으로 안 넘어가기
            if self.velocity > 0 :
                self.velocity = 0
                self.trans()
                bottom = self.rect.bottom
                self.rect.move_ip(0, height - bottom +27)
        elif self.rect.top <= -10 : #위로 안 넘어가기
            if self.velocity < 0:
                self.velocity=0
                self.trans()
                top = self.rect.top
                self.rect.move_ip(0, -top -10)
        self.trans()
